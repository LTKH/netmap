...
export default function () {
  ...
  group('01. Write netstat', () => {
    const data = { "data": [
        { 
          "localAddr": { "ip": `192.168.${ip1}.${ip2}`, "name": `host-n${host}` }, 
    ...
    group('02. Read netstat', () => {
      //console.log("test read host-", host);
      let res = http.get(`http://127.0.0.1:8084/api/v1/netmap/records?src_name=host-n${host}`);
    ...
    group('03. Read netstat', () => {
        //console.log("test read host-", host);
        let res = http.get(`http://127.0.0.1:8086/api/v1/netmap/records?src_name=host-n${host}`);
