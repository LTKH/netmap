...
const (
	EventService_ListObjects_FullMethodName = "/events.EventService/ListObjects"
...
type EventServiceClient interface {
	...
	ListObjects(ctx context.Context, in *ListObjectsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Event], error)
...
func (c *eventServiceClient) ListObjects(ctx context.Context, in *ListObjectsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Event], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &EventService_ServiceDesc.Streams[0], EventService_ListObjects_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ListObjectsRequest, Event]{ClientStream: stream}
...
type EventService_ListObjectsClient = grpc.ServerStreamingClient[Event]
...
type EventServiceServer interface {
	// Запрос всех событий за период времени
	ListObjects(*ListObjectsRequest, grpc.ServerStreamingServer[Event]) error
	// Запрос для подписки (может быть пустым)
...
func (UnimplementedEventServiceServer) ListObjects(*ListObjectsRequest, grpc.ServerStreamingServer[Event]) error {
...
func _EventService_ListObjects_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ListObjectsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(EventServiceServer).ListObjects(m, &grpc.GenericServerStream[ListObjectsRequest, Event]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type EventService_ListObjectsServer = grpc.ServerStreamingServer[Event]
...
var EventService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _EventService_ListObjects_Handler,
			ServerStreams: true,
		},
		{
	...
	Metadata: "proto/netserver.proto",
}
